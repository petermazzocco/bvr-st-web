 ...
 */
export async function createCheckoutSession(initialState: any, formData: FormData) {
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status >= 500) {
        return { error: "Payment system temporarily unavailable. Please try again later." };
      }
    ...
    try {
      ...
      return { error: "Invalid response from payment system. Please try again." };
    }
